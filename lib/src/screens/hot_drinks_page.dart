...
class HotDrinksPage extends StatelessWidget {
  const HotDrinksPage({Key? key}) : super(key: key);

  Future<List<Product>> fetchHotDrinks() async {
    try {
      final response = await http.get(Uri.parse('${ApiConfig.productsUrl}?category=hot_drinks'));
      if (response.statusCode == 200) {
        List<dynamic> productData = json.decode(response.body);
        return productData.map((data) => Product.fromJson(data)).toList();
      } else {
        throw Exception('Failed to load hot drinks');
      }
    } catch (e) {
      throw Exception('Failed to load hot drinks: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Hot Drinks', style: logoWhiteStyle),
      ),
      body: FutureBuilder<List<Product>>(
        future: fetchHotDrinks(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return const Center(child: Text('No hot drinks available'));
          } else {
